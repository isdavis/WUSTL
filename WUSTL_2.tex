...
\invisible<1-7>{\item[2)] $\boldsymbol{\tau}$ is an $N \times K$ matrix. Each row is an indicator vector.\\} \pause
...
\begin{itemize}
\invisible<1-2>{\item[-] Calculate squared euclidean distance from center} \pause %i.e.  for every document, we calculate distance from clustered center
...
\end{itemize}
\invisible<1-8>{\item[-] If $K = 1$, $f(\boldsymbol{X}, \boldsymbol{T}, \boldsymbol{\Theta}) = N \times \sigma^2$} \pause % maximum
\begin{itemize}
